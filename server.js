...
app.get('/api', (req, res) => {
  ...
  res.json({
    ...
    endpoints: [
      ...
      {method: 'POST', path: '/api/books/', description: "Add new book to collection"},
      {method: 'PUT', path: '/api/books/:id', description: "Update book based on id"},
      {method: 'DELETE', path: '/api/books/:id', description: "Delete book based on id"}
    ]
  })
});
...
app.get('/api/profile', (req, res) => {
  res.json({
    'name': 'Özcan Burak Tomruk, Oğuzhan Göller ',
    'homeCountry': 'Turkey',
...
app.put('/api/books/:id', (req, res) => {
  ...
  const {title, author, releaseDate, genre, rating, language} = bookNewData;
...
app.delete('/api/books/:id', (req, res) => {
  ...
  .then((deletedBookInfo) => {
    deletedBook = deletedBookInfo;
  })
  .then(() => res.json(deletedBook));
});
